...
def write_datasets(args):
    ...
    with open(os.path.join(args.target,'validate.txt'), mode='w') as f:
        for item in validfiles:
            new_loc = 'validate/' + item.replace('train/', '')
            os.rename(item, new_loc)
            os.rename(item.replace('.jpg', '.txt'), new_loc.replace('.jpg', '.txt')
            f.write(prefix + new_loc + "\n")
    with open(os.path.join(args.target,'test.txt'), mode='w') as f:
        for item in testfiles:
            new_loc = 'test/' + item.replace('train/', '')
            os.rename(item, new_loc)
            os.rename(item.replace('.jpg', '.txt'), new_loc.replace('.jpg', '.txt')
            f.write(prefix + new_loc + "\n")
